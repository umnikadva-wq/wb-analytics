...
if not df_curr.empty:
    ...
    has_comparison = not df_prev.empty
    ...
    if has_comparison:
        try:
            ...
            st.warning(f"⚠️ Не удалось построить сравнение: {e}. Показываем текущие данные.")
            has_comparison = False
            df_compare = df_curr
    else:
        st.info("ℹ️ Нет данных за предыдущий период для сравнения.")
        df_compare = df_curr
    ...
    if has_comparison:
        ...
        c1.metric("👁 Показы", f"{df_compare['Показы'].sum():,}")
        c2.metric("🖱 Клики", f"{df_compare['Клики'].sum():,}")
        c3.metric("💰 Расход", f"{df_compare['Расход'].sum():,.0f} ₽")
        c4.metric("🛒 Заказы", f"{df_compare['Заказы'].sum():,}")
        c5.metric("💵 Выручка", f"{df_compare['Выручка'].sum():,.0f} ₽")
    ...
    if has_comparison:
        ...
        table = table.rename(columns={
            'Название': 'Кампания',
            'Показы_curr': 'Показы', 'd_Views': 'Δ %',
            'Клики_curr': 'Клики', 'd_Clicks': 'Δ %',
            'Расход_curr': 'Расход', 'd_Cost': 'Δ %',
            'Заказы_curr': 'Заказы', 'd_Orders': 'Δ %',
            'Выручка_curr': 'Выручка', 'd_Revenue': 'Δ %'
        })
        ...
        # Просто таблица без сравнения
        display_df = df_compare[['Название', 'Показы', 'Клики', 'Расход', 'Заказы', 'Выручка', 'CTR', 'CPC', 'ДРР']].copy()
        display_df = display_df.rename(columns={'Название': 'Кампания'})
        
        st.dataframe(
            display_df.style.format({
                'Расход': '{:,.0f} ₽', 'Выручка': '{:,.0f} ₽',
                'CTR': '{:.2f}%', 'CPC': '{:.2f} ₽', 'ДРР': '{:.1f}%'
            }),
            use_container_width=True,
            height=500
        )
    ...
    if not TOKEN:
        ...
        st.info("👈 Нажмите ' Обновить данные' слева.")
