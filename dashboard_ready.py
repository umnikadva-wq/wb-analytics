...
# Загрузка данных
if TOKEN and ("refresh" in st.session_state or "df_curr" not in st.session_state):
...
if not df_curr.empty:
    # --- СРАВНЕНИЕ (MERGE) ---
    # Объединяем текущее и прошлое по ID кампании
    ...
    if not TOKEN:
        st.error("❌ Ошибка: Токен не найден. Проверьте Secrets в Streamlit Cloud.")
    else:
